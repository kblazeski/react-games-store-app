...
export const signUp = (email,pw) => {
    return dispatch => {
        ...
        axios.post('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key='+webApiKey,authData)
            .then(res => {
                const data = {
...
export const login = (email,pw) => {
    return dispatch => {
        ...
        const webApiKey = 'AIzaSyCKdky09sCJnnlde2kCTQ_L7RZQyQMS0VY';
        axios.post('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key='+webApiKey,authData)
            .then(res => {
                const data = {
                    userId: res.data.localId,
                    idToken: res.data.idToken,
